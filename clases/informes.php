<?php 
    ...
    class graficas extends Conexion {
        ...
        public function reporte_ohtenco($year){
	    	$total = array();
	    	for($i=0; $i<12; $i++){
	    		$month = $i+1;
				$consulta = "SELECT COUNT(id_delito) AS total FROM t_reportes WHERE MONTH(fecha) = '$month' AND YEAR(fecha) = '$year' AND id_delito=7 AND id_poblado = 1 LIMIT 1";
	    		$sql = $this->conectar()->query($consulta);	
	    		$total[$i] = 0;
	    		foreach ($sql as $key){ $total[$i] = ($key['total'] == null)? 0 : $key['total']; }
	    	}			 
        ...
        public function reporte_tepenahuac($year){
	    	$total = array();
	    	for($i=0; $i<12; $i++){
	    		$month = $i+1;
				$consulta = "SELECT COUNT(id_delito) AS total FROM t_reportes WHERE MONTH(fecha) = '$month' AND YEAR(fecha) = '$year' AND id_delito=7 AND id_poblado = 6 LIMIT 1";
	    		$sql = $this->conectar()->query($consulta);	
	    		$total[$i] = 0;
	    		foreach ($sql as $key){ $total[$i] = ($key['total'] == null)? 0 : $key['total']; }
	    	}			 
        ...
        public function reporte_tlacotenco($year){
	    	$total = array();
	    	for($i=0; $i<12; $i++){
	    		$month = $i+1;
				$consulta = "SELECT COUNT(id_delito) AS total FROM t_reportes WHERE MONTH(fecha) = '$month' AND YEAR(fecha) = '$year' AND id_delito=7 AND id_poblado = 11 LIMIT 1";
	    		$sql = $this->conectar()->query($consulta);	
	    		$total[$i] = 0;
	    		foreach ($sql as $key){ $total[$i] = ($key['total'] == null)? 0 : $key['total']; }
	    	}			 
        ...
        public function report_buil($year){
            ...
            $ohtencosql = $this->reporte_ohtenco($year);
            $tepenahuacsql = $this->reporte_tepenahuac($year);
            $tlacotencosql = $this->reporte_tlacotenco($year);
